...
void chain_estimator::reset(const YAML::Node &node){
    ...
    for (int k=0; k<N; k++) {
        ...
        // Pull off the first packet to avoid EKF issues
        float dt;
        float meas[13];
        while (!imu_vec[k]->IMU.getStream(meas, &dt))
            sleep(1/3000);
    }
...
int chain_estimator::update(){
    if (isInit) {
        ...
        for (int i = 0; i<N; i++) {
            ...
            while (!imu_vec[i]->IMU.getStream(meas, &dt))
                sleep(1/3000);
...
int chain_estimator::getState(int i, float * state, float * meas, float * proximal_acc){
    state           = imu_vec[i]->last_measurement.data();
    proximal_acc    = imu_vec[i]->acc.data();
}
