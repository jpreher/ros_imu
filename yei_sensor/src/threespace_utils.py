...
"""This Module is a collection of utility functions used in the ThreeSpace API.

The ThreeSpaceUtils module is a collection of functions, structures, and static
variables use exclusivly in the ThreeSpaceAPI module. All functions in this
module are used to find available ThreeSpace devices on the host system and
information on them. This module can be used with a system running Python 2.5
and newer (including Python 3.x).
"""
...
def pyTryPort(port_name, conn):
    try:
        tmp_port = serial.Serial(   port_name,
                                    timeout=0.2,
                                    writeTimeout=0.2,
                                    baudrate=115200)
    except:
...
def checkSoftwareVersionFromPort(serial_port):
    ...
    if len(response) == 0:
        ...
        remainder = convertString(serial_port.read(3))
